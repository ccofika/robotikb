      ...
      if (search.length > 1) {
        ...
        const [equipmentUsers, workOrderUsers] = await Promise.all([
          ...
          Equipment.distinct('location', {
            serialNumber: { $regex: looseTextRegex(search), $options: 'i' },
            location: { $regex: /^user-/ }
          }),
          // Find users by work order data (using correct field names)
          WorkOrder.distinct('user', {
            $or: [
              { tisJobId: { $regex: looseTextRegex(search), $options: 'i' } },
              { details: { $regex: looseTextRegex(search), $options: 'i' } },
              { tisId: { $regex: looseTextRegex(search), $options: 'i' } }
            ]
          })
        ]);
        ...
        const searchConditions = [
          ...
          { name: { $regex: looseTextRegex(search), $options: 'i' } },
          { address: { $regex: looseTextRegex(search), $options: 'i' } },
          { phone: { $regex: looseTextRegex(search), $options: 'i' } },
          { tisId: { $regex: looseTextRegex(search), $options: 'i' } }
        ];
